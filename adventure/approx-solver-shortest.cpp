...
int main()
{
    ...
    typedef std::tuple<int, int, int, int, int, unsigned long long> elem;
    std::priority_queue<elem, std::vector<elem>, std::greater<elem>> q;
    for (int i = 0; i < n; i++)
        q.push({0, 0, i, i, i, 0});
    while (!q.empty())
    {
        auto [colors, length, start, prev, cur, mask] = q.top();
        q.pop();
        ...
        for (int neighbor : neighbors[cur])
        {
            ...
            q.push({__builtin_popcountll(m2), length + 1, start, cur, neighbor, m2});
        }
    }
